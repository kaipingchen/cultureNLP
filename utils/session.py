...
def session_setup():
    ...
    if get_session_state('submitted_once') is None:
        set_session_state('submitted_once', False)
...
# Get session state value og key
...
# Set session state value of key
...
def intro_response():
    data = {
        ...
        "temperature": float(os.getenv('intro_temp')),
        "top_p": float(os.getenv('intro_top_p')),
        "repetition_penalty": float(os.getenv('intro_rep_penalty')),
        "max_tokens": int(os.getenv("intro_max_tokens"))
    }
