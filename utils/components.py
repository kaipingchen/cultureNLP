...
from utils.database import handle_new_submission
import random
...
def show_response_count():
    ...
    if response_count < int(os.getenv('warning_responses')):
        st.info(response_count_message)

    if int(os.getenv('warning_responses')) <= response_count < int(os.getenv('maximum_responses')):
        convo_warning_1 = os.getenv('convo_warning_1')
        convo_warning_2 = os.getenv('convo_warning_2')
        extra_count = int(os.getenv('maximum_responses')) - response_count
        response_count_message += f'{convo_warning_1} {extra_count} {convo_warning_2}'
        st.warning(response_count_message)

    if int(os.getenv('maximum_responses')) <= response_count:
        st.success(response_count_message)


# Display done button when sufficient feedback provided
def submit_button(submit_button_disabled):
...
# Enable emoji reactions for each response
...
def comments():
    ...
    submit_button(submit_button_disabled)
...
# Button to finis the chat portion of the app
def finish_button():
    response_count = get_session_state('response_count')

    if response_count >= int(os.getenv('maximum_responses')):
    ...
    if response_count >= int(os.getenv('minimum_responses')):
